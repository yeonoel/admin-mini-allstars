...
import {
  ...
  filterButtons,
  StatusCanBeChanged,
} from "@/components/features/Order/order-config";
...
import {
  canBeUpdate,
  canChangeTo,
} from "@/components/features/Order/order-status-managment";
      ...
      <div className="flex overflow-x-auto gap-2 pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap">
        {filterButtons.map((filter) => (
          <button
            ...
            onClick={() =>
              setStatusFilter(
                statusFilter === filter.status ? null : filter.status,
              )
            }
                        ...
                        <div className="flex flex-col items-center gap-2">
                          ...
                          {canBeUpdate(order.status) && (
                            ...
                            >
                              ...
                              <SelectContent>
                                {Object.entries(StatusCanBeChanged).map(
                                  ([status, config]) => (
                                    <SelectItem
                                      key={status}
                                      value={status}
