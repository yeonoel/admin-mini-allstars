...
export const storesApi = {
  create: async (dto: CreateStoreDto): Promise<CreateStoreResponse> => {
    try {
      ...
      const { data } = await apiClient.post<CreateStoreResponse>(
        "/stores/create",
        formData,
        {
          headers: {
  ...
  update: async (
    ...
    if (dto.whatsappNumber)
      formData.append("whatsappNumber", normalizePhone(dto.whatsappNumber));
    if (dto.description !== undefined)
      formData.append("description", dto.description ?? "");
    if (dto.logo) formData.append("logo", dto.logo);
    const { data } = await apiClient.patch<CreateStoreResponse>(
      `/stores/${id}`,
      formData,
      {
